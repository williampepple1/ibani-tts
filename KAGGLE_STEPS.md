...
Since Kaggle sometimes updates its default Python version, we will create a dedicated Python 3.10 environment to ensure compatibility with TTS using `uv`.
```bash
...
```bash
!/kaggle/venv/bin/python kaggle_prep_script.py
```
...
```bash
!/kaggle/venv/bin/pip install TTS
```

### Cell 6: Start Training!
Run the training script we prepared. 
```bash
!/kaggle/venv/bin/python train_tts.py
```
